...
int main(int argc, char** argv) {
  ...
  for (const auto& [key, val] : data["worldPoints"].items()) {
    points[key].xyz[0] = uniform_dist(gen);
    points[key].xyz[1] = uniform_dist(gen);
    points[key].xyz[2] = uniform_dist(gen);
  }
  ...
  for (const auto& [camKey, camVal] : data["cameras"].items()) {
    ...
    // fx  = fy
    double fx = uniform_dist(gen);
    double cx = uniform_dist(gen);
    double cy = uniform_dist(gen);
    ...
    for (int r = 0; r < 3; ++r) {
      for (int c = 0; c < 3; ++c) {
        rotation_matrix[r * 3 + c] = uniform_dist(gen);
      }
      // Translation vector is the 4th column
      cam.params[4 + r] = uniform_dist(gen);
    }
  ...
  ba::json_write(points, cameras, output_data, "answer_only_obs.json");
